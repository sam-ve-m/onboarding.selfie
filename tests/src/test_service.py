...
from func.src.domain.exceptions import SelfieNotExists, ErrorOnSendAuditLog, InvalidOnboardingCurrentStep
from func.src.services.selfie import SelfieService
from tests.src.stubs import stub_content, stub_unique_id
...
async def test_when_failed_to_send_audit_log_then_raises(
    ...
    with pytest.raises(ErrorOnSendAuditLog):
        await SelfieService.save_user_selfie(
            selfie_validated=image_b64, unique_id=stub_unique_id
        )
...
async def test_when_current_step_correct_then_return_true(mock_onboarding_steps):
    result = await SelfieService.validate_current_onboarding_step(
        jwt="123"
    )
