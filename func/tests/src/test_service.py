...
from func.src.domain.exceptions import SelfieNotExists, ErrorOnSendAuditLog
from func.src.services.selfie import SelfieService
from .stubs import stub_content, stub_unique_id
...
async def test_when_failed_to_send_audit_log_then_raises(mock_persephone, mock_save_user, mock_content):
    with pytest.raises(ErrorOnSendAuditLog):
        await SelfieService.save_user_selfie(
            selfie_validated=image_b64,
...
async def test_when_image_as_str_then_return_temp_file():
    result = await SelfieService._resolve_content(selfie_validated=image_b64)

    assert isinstance(result, BufferedRandom)
    assert result.__sizeof__() == 152


@pytest.mark.asyncio
@patch('func.src.services.selfie.Audit.register_log')
@patch.object(SelfieService, '_content_exists')
@patch('func.src.services.selfie.FileRepository.save_user_file')
async def test_when_valid_unique_id_and_selfie_then_return_true(mock_save_file, mock_content_exists, mock_register_log):
...
@patch.object(SelfieService, '_content_exists')
@patch('func.src.services.selfie.FileRepository.save_user_file')
async def test_when_valid_unique_id_and_selfie_then_mocks_was_called(mock_save_file, mock_content_exists, mock_register_log):
    await SelfieService.save_user_selfie(
        selfie_validated=image_b64,
        unique_id=stub_unique_id
    )
